import { FileSearch, CalendarClock } from 'lucide-react';
      ...
      {stats.map(stat => {
        ...
        return (
          <button
            ...
            className={`group surface-card p-5 text-left rounded-xl cursor-pointer transition-colors hover:bg-white/[0.03] hover:border-primary/30 ${
          ...
          >
            ...
            <p className="text-[36px] md:text-[52px] font-display font-bold text-foreground leading-none">{stat.value}</p>
          </button>
        );
      })}
    </div>
