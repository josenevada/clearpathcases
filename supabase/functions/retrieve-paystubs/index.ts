...
serve(async (req) => {
  ...
  try {
    ...
    await stagehand.page.goto('https://my.adp.com/#/pay/statements', {
      waitUntil: 'domcontentloaded',
    });
    await stagehand.page.waitForTimeout(3000);

    try {
      await stagehand.page.act('If a popup about paperless settings appears, close it.');
      await stagehand.page.waitForTimeout(1000);
    } catch (_) {}

    for (let i = 0; i < 4; i++) {
      try {
        const label = i === 0 ? 'most recent' : ['second', 'third', 'fourth'][i - 1];
        await stagehand.page.act(`Click the ${label} pay statement entry in the list.`);
        await stagehand.page.waitForTimeout(1500);
        await stagehand.page.act('Click the "View statement" button.');
        await stagehand.page.waitForTimeout(1000);
        await stagehand.page.act('Click "Download current statement" from the dropdown.');
    ...
    if (filesRes.ok) {
      ...
      for (const file of pdfFiles) {
        try {
          ...
          const fileName = file.name || file.path?.split('/').pop() || `PayStub-${Date.now()}.pdf`;
    ...
    if (uploadedFiles.length === 0) {
      ...
      if (zipRes.ok) {
        ...
        if (buffer.byteLength > 100) {
          await uploadBuffer(buffer, `PayStubs-${Date.now()}.zip`, 'application/zip');
        }
      }
    }
